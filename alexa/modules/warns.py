...
from alexa.modules.helper_funcs.chat_status import is_user_admin, bot_admin, user_admin_no_reply, user_admin, \
    user_can_restrict, user_can_ban
from alexa.modules.helper_funcs.extraction import extract_text, extract_user_and_text, extract_user
...
def button(bot: Bot, update: Update) -> str:
    ...
    if match:
        ...
        if res:
            ...
            update.effective_message.edit_text(
                "Person already has no warns.".format(mention_html(user.id, user.first_name)),
                parse_mode=ParseMode.HTML)
...
def warns(bot: Bot, update: Update, args: List[str]):
    ...
    if result and result[0] != 0:
        ...
        if reasons:
            text = "This person has {}/{} warnings, for the following reasons:".format(num_warns, limit)
            ...
            update.effective_message.reply_text(
                "Person has {}/{} warnings, but no reasons for any of them.".format(num_warns, limit))
    else:
...
MYWARNS_HANDLER = CommandHandler("warns", warns, pass_args=True, filters=Filters.group)
...
LIST_WARN_HANDLER = CommandHandler(["warnlist", "warnfilters"], list_warn_filters, filters=Filters.group)
